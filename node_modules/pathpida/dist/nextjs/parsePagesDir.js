...
const createMethods = (indent, importName, slugs, pathname) => `${indent}  $url: ` +
    ...
    ` => ({ pathname: '${pathname}' as const${slugs.length
...
const parsePagesDir = (input, output, ignorePath, pageExtensions = ['tsx', 'ts', 'jsx', 'js']) => {
    ...
    const createPathObjString = (targetDir, indent, url, slugs, text, methodsOfIndexTsFile) => {
        ...
        const props = fs_1.default
            ...
            let valFn = `${indent}${JSON.stringify((0, replaceWithUnderscore_1.replaceWithUnderscore)(basename))}: {\n<% next %>\n${indent}}`;
